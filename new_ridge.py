...
alpha_range = np.logspace(1, 2, 20)
...
ridge_final = Ridge(alpha=best_alpha, max_iter=10000, random_state=42)
ridge_final.fit(x_train_new, y_train)
y_pred_test_final = ridge_final.predict(x_test_new)
y_pred_train_final = ridge_final.predict(x_train_new)

explainer = shap.LinearExplainer(ridge_final, x_train_df)
shap_values = explainer(x_train_df)
plt.figure(figsize=(15, 10))
shap.summary_plot(shap_values, x_train_df)
plt.title("Feature Importance Plot For Ridge")
plt.show()
...
plt.title('Comparison of Metrics for Train and Test Data in Ridge with Genetic Programming')
plt.legend()
...
plt.title('Actual vs. Predicted Values for Ridge with Genetic Programming')
plt.legend()
plt.show()
...
Y_sensitivity = ridge_final.predict(param_values)
...
sensitivity_df.to_csv("ridge_sensitivity_analysis.csv", index=False)

plt.figure(figsize=(12, 6))
plt.barh(feature_names, Si['ST'], color='skyblue', edgecolor='black')
plt.xlabel("Total Sensitivity Index (ST)")
plt.ylabel("Feature")
plt.title("Feature Sensitivity Analysis (Sobol Indices) using Ridge model")
